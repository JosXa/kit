...
let knodePath = (...parts) =>
  path.join(process.env.KNODE || path.resolve(homedir(), '.knode'), ...parts.filter(Boolean))
...
if (nodeExists) {
  ...
  // Check node version
  let { stdout: nodeVersion } = await exec(`${knodePath('bin', 'node')} --version`)
  console.log(`Current knode version: ${nodeVersion}. Required version ${knodeVersion}`)
...
let { stdout: nodeVersion } = await exec(`${knodePath('bin', 'node')} --version`, options)
let { stdout: npmVersion } = await exec(`${knodePath('bin', 'npm')} --version`, options)
...
let dec = exec(`${knodePath('bin', 'npx')} tsc --project ./tsconfig-declaration.json --outDir ${kitPath()}`).catch(
  (e) => {
    console.error(e)
    process.exit(1)
  },
...
await exec(`${knodePath('bin', 'node')} ./run/terminal.js ./help/download-docs.js`, options)

console.log('Download hot')
await exec(`${knodePath('bin', 'node')} ./run/terminal.js ./hot/download-hot.js`, options)
