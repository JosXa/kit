...
let moveKenv = await arg('Move ~/.kenv to directory?', ['no', 'yes'])
if (moveKenv === 'yes') {
  await move(kenvPath(), path.join(newKenvParentPath, '.kenv'))
  await replace({
...
await sendWait(Channel.KENV_NEW_PATH, path.join(newKenvParentPath, '.kenv'))
