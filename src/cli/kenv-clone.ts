...
${repo ? `> Before proceeding, please review the scripts folder here: [${repo}](${repo})` : '> Enter a valid repo URL'}
...
let [repo, kenvName, removeGit, install, ok] = await fields({
  ...
  onInit: async () => {
    if (initRepo) {
      ...
      if (kenvName === '.kenv' || kenvName === 'kenv') {
        kenvName = path.basename(path.dirname(repo)) + '-kenv'
      }
      setFormData({
        repo,
        kenvName,
  ...
  onChange: async (i, s) => {
    let [repo, kenvName, removeGit, install, ok] = s?.value ?? []
    if (ok === 'ok' && kenvName) {
      if (removeGit === 'y') {
...
let packageJsonPath = kenvPath('kenvs', kenvName, 'package.json')
