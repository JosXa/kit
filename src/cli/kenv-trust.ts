...
let preview = md(`# <span class="text-red-500 text-3xl animate-pulse">Danger Zone<span>
...
let addPreview = async (s) => {
  s.preview = async () => {
    ...
    if (s.snippet) {
      detected += `* Snippet: ${s.snippet}\n`
    }
    if (s.watch) {
      detected += `* Watch: ${s.watch}\n`
    }
    if (s.background) {
    ...
    let js = await highlightJavaScript(preview, s?.shebang || '')
...
let finalScripts = await Promise.all(autoScripts.map(addPreview))

let warningChoice = {
  name: 'EXTREME CAUTION',
  description: 'This kenv contains scripts which run automatically',
  preview,
}
...
let cliKenv = await arg(matchKenvConfig, async (input) => choices)
