...
let packages = (await arg(
  ...
  [...Object.keys(file?.dependencies || []), ...Object.keys(file?.devDependencies || [])].filter(
    (k) => !k.startsWith('@johnlindquist/kit'),
  ),
)) as string[]
...
let [tool, command] = (isYarn ? `yarn${global.isWin ? '.cmd' : ''} add` : `npm${global.isWin ? '.cmd' : ''} i`).split(
  ' ',
)
...
await term({
  command: `${PATH}${tool} ${command} ${packages.map((p) => `${p}@latest`).join(' ')}`.trim(),
  env: {
    ...global.env,
    PATH: KIT_FIRST_PATH,
  },
