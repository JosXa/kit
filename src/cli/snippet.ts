...
import { checkIfCommandExists, kitMode, returnOrEnter, uniq } from '../core/utils.js'
import { stripMetadata } from '../core/utils.js'
import { ensureTemplates, prependImport } from './lib/utils.js'

let previewContent = ''
if (arg?.content) {
  previewContent = Buffer.from(arg.content, 'base64url').toString()
  if (!arg?.keepMetadata) {
...
let panel = await highlight(header + previewContent, containerClasses)
...
name = name
  ...
  .replace(/\s/g, '-')
  .toLowerCase()
let scriptPath = path.join(selectedKenvPath, 'scripts', `${name}.${kitMode()}`)
...
if (arg?.content) {
  contents = Buffer.from(arg.content, 'base64url').toString()
  if (!arg?.keepMetadata) {
