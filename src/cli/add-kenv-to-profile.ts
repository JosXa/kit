...
4. In the next screen, click "New" and then "Browse" to find the ${kenvPath('bint')} directory.
...
let exportKenvPath = `export PATH="$PATH:${path.resolve(kenv.dirPath, 'bin')}"`

let choices = ['.zshrc', '.bashrc', '.bash_profile', '.config/fish/config.fish', '.profile']

let profiles = choices.map((profile) => `${env.HOME}/${profile}`).filter((profile) => existsSync(profile))
