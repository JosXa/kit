...
let packageNames = (await arg(
  ...
  [...Object.keys(file?.dependencies || []), ...Object.keys(file?.devDependencies || [])].filter(
    (k) => !k.startsWith('@johnlindquist'),
  ),
)) as string[]
...
let [tool, toolArgs] = (
  isYarn ? `yarn${global.isWin ? '.cmd' : ''} remove` : `npm${global.isWin ? '.cmd' : ''} un`
).split(' ')

let toolPath = global.isWin ? (isYarn ? 'yarn' : knodePath('bin', 'npm.cmd')) : `PATH=${knodePath('bin')}:$PATH ${tool}`
