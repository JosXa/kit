...
let customThemePaths = await globby([kenvPath('themes', '*.json').replaceAll('\\', '/')])
...
let defaultThemePaths = await globby([kitPath('themes', 'script-kit*.json').replaceAll('\\', '/')])
...
if (themePath === 'theme-designer') {
  ...
  if (themePath === RESET) {
    ...
    let appearance = await arg(`Use ${themeName} When Appearance`, ['Light', 'Dark', 'Both'])
