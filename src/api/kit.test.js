...
/** @type {import("./utils")} */
let { resolveToScriptPath } = await import(kitPath('core', 'utils.js'))
...
ava.serial(`testing "run" is global`, async (t) => {
  ...
  let { stdout, stderr } = await $`${kenvPath('bin', mainScript)}`
...
ava.serial('tmpPath generates a tmp path', async (t) => {
  ...
  t.is(stdout.trim(), path.resolve(os.tmpdir(), 'kit', script, file))
  t.is(stderr, '')
})
...
ava.serial('setEnvVar sets an environment variable', async (t) => {
  ...
  const value = 'hello'
  const { setEnvVar } = await import(kitPath('api', 'kit.js'))
...
ava.serial('getEnvVar gets an environment variable', async (t) => {
  ...
  let { setEnvVar, getEnvVar } = await import(kitPath('api', 'kit.js'))
...
ava.serial('getEnvVar returns fallback if environment variable does not exist', async (t) => {
  let key = 'NON_EXISTENT_ENV_VAR'
  let fallback = 'default'

  // We attempt to retrieve a non-existent environment variable
  let { getEnvVar } = await import(kitPath('api', 'kit.js'))
  let value = await getEnvVar(key, fallback)
...
ava.serial('toggleEnvVar toggles an environment variable', async (t) => {
  ...
  /** @type {import("./kit.js")} */
  let { setEnvVar, toggleEnvVar, getEnvVar } = await import(kitPath('api', 'kit.js'))
