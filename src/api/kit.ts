...
import { type ConfigOptions, type Options, QuickScore, createConfig, quickScore } from 'quick-score'
...
import { type Stamp, getScriptFromString, getScripts, getTimestamps, getUserJson, setUserJson } from '../core/db.js'
import {
  formatChoices,
  getKenvs,
  groupChoices,
...
export let errorPrompt = async (error: Error) => {
  ...
  if (process.env.KIT_CONTEXT === 'app') {
    global.warn('☠ ERROR PROMPT SHOULD SHOW ☠')
  ...
  return (query: string) => {
    ...
    if (result.find((c) => c?.item?.group)) {
      ...
      result = filteredBySearch.filter((sc) => {
        if (sc?.item?.skip) {
          if (!keepGroups.has(sc?.item?.group)) {
            return false
          }
        }
...
let getLanguage = (language: string) => {
  ...
  if (language.includes('ruby')) {
    return 'ruby'
  }
  if (language.includes('php')) {
    return 'php'
  }
  if (language.includes('perl')) {
    return 'perl'
  }
  ...
  'edit-script': {
    ...
    preview: async (input, state) => {
      ...
      let compileStamp = stamp?.compileStamp
        ? `Last compiled: ${formatDistanceToNow(new Date(stamp?.compileStamp), { includeSeconds: true })} ago`
        : ''
...
let groupScripts = (scripts) => {
  ...
  return groupChoices(scripts, {
    ...
    recentKey: 'timestamp',
    excludeGroups,
    recentLimit: process?.env?.KIT_RECENT_LIMIT ? Number.parseInt(process.env.KIT_RECENT_LIMIT, 10) : 3,
...
export let getProcessedScripts = async (fromCache = true) => {
  ...
  processedScripts = await processInBatches(scripts.map(processScript(timestampsDb.stamps)), 10)
...
export let getGroupedScripts = async (fromCache = true) => {
  ...
  if (isMac) {
    ...
    if (!global?.env?.KIT_ACCESSIBILITY) {
  ...
  if (hasLinks) {
    ...
    let linksScriptsWithScriptValues = linksScripts.map((s) => {
      s.value = Object.assign({}, s)
      return s
    })
    processedscripts = processedscripts.concat(linksScriptsWithScriptValues)
  }
  ...
  async (s: Script): Promise<Script> => {
    ...
    if (stamp) {
      ...
      if (stamp.compileMessage && stamp.compileStamp) {
        infoBlock = `~~~
! Last compiled ${formatDistanceToNow(new Date(stamp.compileStamp))} ago
...
export let processWithStringPreview = async (s: Script, infoBlock: string) => {
  ...
  if (s?.preview === 'false') {
    ...
    try {
      ...
      processedPreview = infoBlock ? md(infoBlock) : '' + (await highlight(content))
    } catch (error) {
...
global.highlight = async (markdown: string, containerClass = 'p-5 leading-loose', injectStyles = '') => {
  let { default: highlight } = global.__kitHighlight || (await import('highlight.js'))
  if (!global.__kitHighlight) {
    global.__kitHighlight = { default: highlight }
  }
...
global.md = (content = '', containerClasses = 'p-5 prose prose-sm') => {
