...
let findMain = async (parent, packageName: string, packageJson: PackageJson) => {
  try {
    let kPath = (...pathParts: string[]) => global.kenvPath(parent, 'node_modules', packageName, ...pathParts)
...
let findPackageJson =
  (packageName: string) =>
  async (parent = '') => {
    let packageJson = global.kenvPath(parent, 'node_modules', packageName, 'package.json')
...
export let createNpm =
  ...
  async (packageNameWithVersion) => {
    ...
    let { dependencies: kitDeps = {}, devDependencies: devDeps = {} } = JSON.parse(
      await global.readFile(global.kitPath('package.json'), 'utf-8'),
    )
    ...
    let { dependencies: kenvDeps = {}, devDependencies: kenvDevDeps = {} } = JSON.parse(
      await global.readFile(pkgPath, 'utf-8'),
    )
