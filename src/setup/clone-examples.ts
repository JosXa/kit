...
if (await isDir(examplesDir)) {
  ...
  if (await isBin('git')) {
    await exec('git clone https://github.com/johnlindquist/kit-examples examples', {
      cwd: kenvPath('kenvs'),
    })
  } else {
    let destination = kenvPath('tmp')
    await download('https://github.com/johnlindquist/kit-examples/archive/main.zip', destination, {
      extract: true,
