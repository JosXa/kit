...
let { isFile, KIT_FIRST_PATH } = await import(kitPath('core', 'utils.js'))

let KIT = kitPath()
let KENV = kenvTestPath
let KNODE = process.env.KNODE || path.resolve(os.homedir(), '.knode')
...
ava.serial('env was created', async (t) => {
  ...
  let contents = await readFile(kenvSetupMockPath('.env'), 'utf-8')
...
ava.serial('kenv linked to kit', async (t) => {
  ...
  t.assert(pkg.devDependencies?.['@johnlindquist/kit'], 'file:../.kit', 'kenv linked to kit')
})
