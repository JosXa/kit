...
/** @type {import("./utils")} */
let { resolveToScriptPath, run } = await import(kitPath('core', 'utils.js'))
...
ava.serial('run full path', async (t) => {
  ...
  let testingRunPackageJson = kitMockPath('.kit-testing-run-path', 'package.json')
...
ava.serial('run full path with spaces', async (t) => {
  let testingSpacedPath = kitMockPath('.kit testing spaced path', 'some-script.js')

  let testingSpacedPackageJson = kitMockPath('.kit testing spaced path', 'package.json')
...
ava.serial('resolve ./scripts dir', async (t) => {
  ...
  let mockScriptInProjectScript = path.resolve(mockScriptsInProject, 'scripts', `${script}.js`)
...
ava.serial("projectPath return the directory of the script's project", async (t) => {
