...
import { PathLike, lstatSync, realpathSync } from 'node:fs'
...
import type { Choice, Metadata, Script, ScriptMetadata, ScriptPathInfo, Shortcut } from '../types/core'
...
import { Channel, PROMPT, ProcessType } from './enum.js'
...
//app
export let kitPath = (...parts: string[]) => path.join(process.env.KIT || home('.kit'), ...parts.filter(Boolean))
...
export let knodePath = (...parts: string[]) => path.join(process.env.KNODE || home('.knode'), ...parts.filter(Boolean))
...
//app
export let resolveToScriptPath = (script: string, cwd: string = process.cwd()): string => {
...
//app
export let postprocessMetadata = (metadata: Metadata, fileContents: string): ScriptMetadata => {
  ...
  let tabs = fileContents.match(/(?<=^onTab[(]['"]).+?(?=['"])/gim) || []
...
//app
export let commandFromFilePath = (filePath: string) => path.basename(filePath)?.replace(/\.(j|t)s$/, '') || ''
...
//app
export let parseFilePath = async (filePath: string): Promise<ScriptPathInfo> => {
...
export let kenvFromFilePath = (filePath: string) => {
  ...
  if (path.relative(kenvDir, kenvPath()) === '') {
    return ''
  }
  return kenv
}
...
export let trashScriptBin = async (script: Script) => {
  ...
  if (binJS) {
    let binPath = jsh ? kenvPath('node_modules', '.bin', command) : commandBinPath
...
export let groupChoices = (choices: Choice[], options = {}) => {
  ...
  let putIntoGroups = (choice: Choice) => {
    ...
    if (excludeGroups.find((c) => choice?.group === c || (choice as Script)?.kenv === c)) {
      choice.exclude = true
    }
    if (choice?.pass) {
      choice.group = 'Pass'
  ...
  for (let choice of choices) {
    if (choice[recentKey] && !choice.pass && !(typeof choice?.recent === 'boolean' && choice?.recent === false)) {
    ...
    if (recentGroup.choices.length > recentLimit) {
      ...
      unrecentGroup = recentGroup.choices.splice(recentLimit, recentGroup.choices.length - recentLimit)
    }
...
export let formatChoices = (choices: Choice[], className = ''): Choice[] => {
  if (Array.isArray(choices)) {
    return (choices as Choice<any>[]).flatMap((choice, index) => {
      ...
      properChoice.choices = undefined

      let isArray = Array.isArray(choiceChoices)
      if (!isArray) {
        throw new Error(`Group choices must be an array. Received ${typeof choiceChoices}`)
      }

      let groupedChoices = []

      properChoice.group = properChoice.name
      properChoice.skip = typeof choice?.skip === 'undefined' ? true : choice.skip
...
export let processInBatches = async <T>(items: Promise<T>[], batchSize: number): Promise<T[]> => {
  let result: T[] = []
...
export let parseMarkdownAsScripts = async (markdown: string): Promise<Script[]> => {
  ...
  for (const untrimmedLine of lines) {
    ...
    if (line.startsWith('```submit') || line.startsWith('~~~submit')) {
      parsingValues = true
      continue
    }

    if ((parsingValues && line.startsWith('```')) || line.startsWith('~~~')) {
      parsingValues = false
