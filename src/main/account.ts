...
import { cmd, escapeShortcut, proPane, userDbPath } from '../core/utils.js'
setChoices([])
...
`),
  ...
  switch (option) {
    case 'pro':
      ...
      break
    case 'discord': {
      let response = await post('https://scriptkit.com/api/discord-invite', await getUserJson())
      open(response.data)
      break
    }
    case 'pro-status': {
...
<a href="submit:login" class="text-black font-bold px-3 py-3 h-6 no-underline rounded bg-white bg-opacity-80  hover:opacity-75">Sign in with GitHub</a>
