...
import type { AppState, ChannelHandler, Choice, PromptConfig, PromptData } from '../types/core'
...
import { compareAsc, formatDistanceToNow } from '@johnlindquist/kit-internal/date-fns'
...
import { Channel, Key, Mode, PROMPT, UI, Value } from '../core/enum.js'
...
let createHandlerWrapper = (channel: keyof ChannelMap, handler: (data: any) => void) => {
...
global.onMousedown = (handler) => {
  return createHandlerWrapper(Channel.SYSTEM_MOUSEDOWN, handler)
}

global.onMouseup = (handler) => {
  return createHandlerWrapper(Channel.SYSTEM_MOUSEUP, handler)
}

global.onMousemove = (handler) => {
  return createHandlerWrapper(Channel.SYSTEM_MOUSEMOVE, handler)
}
...
global.onKeydown = (handler) => {
  return createHandlerWrapper(Channel.SYSTEM_KEYDOWN, handler)
}
...
global.app = {
  onScriptChanged: (handler) => {
    return createHandlerWrapper(Channel.SCRIPT_CHANGED, handler)
  },
  onScriptAdded: (handler) => {
    return createHandlerWrapper(Channel.SCRIPT_ADDED, handler)
  },
  onScriptRemoved: (handler) => {
    return createHandlerWrapper(Channel.SCRIPT_REMOVED, handler)
  },
}
...
let displayChoices = async ({ choices, className, scripts, generated, inputRegex }: DisplayChoicesProps) => {
...
let invokeChoices = async (props: InvokeChoicesProps) => {
  ...
  let resultOrPromise = (props.choices as Function)(props.state.input)

  if (resultOrPromise?.then) {
    let result = await resultOrPromise
    if (localInvocation !== invocation) {
      return
    }
    if (props.promptId === promptId && props.tabIndex === global.onTabIndex) {
      displayChoices({
        ...props,
        choices: result,
...
interface WaitForPromptValueProps extends DisplayChoicesProps {
...
let createOnChoiceFocusDefault = (debounceChoiceFocus: number, onUserChoiceFocus?: ChannelHandler) => {
  let _promptId = promptId
  let debouncedChoiceFocus = debounce(async (input: string, state: AppState = {}) => {
    if (_promptId !== promptId) {
      return
    }
    let preview = ''
    ...
    let currentChoices = (global?.kitPrevChoices || []).concat(global?.kitFlagsAsChoices || [])
    ...
    if (global?.__currentPromptConfig?.shortcuts) {
      const shortcuts = global?.__currentPromptConfig?.shortcuts?.filter((shortcut) => {
...
let findAction = async (data: AppMessage) => {
  if (data?.state?.action?.name) {
    let f = global.kitFlagsAsChoices?.find((f) => f?.name === data?.state?.action?.name)
    if (f?.onAction) {
...
let waitForPromptValue = ({
  ...
  global.activePromptPromise = new Promise((resolve, reject) => {
    if (ui === UI.arg || ui === UI.hotkey || ui === UI.div) {
      getInitialChoices({
        promptId,
    ...
    ).pipe(takeUntil(global.__kitEndPrevPromptSubject), share())
    ...
    let valueSubmitted$ = message$.pipe(
      filter((data) => data.channel === Channel.VALUE_SUBMITTED),
      share(),
    )
    ...
    message$.pipe(takeUntil(value$), share()).subscribe({
      next: async (data) => {
        ...
        switch (data.channel) {
          ...
          case Channel.SHORTCUT: {
            ...
            const shortcut = (global.__currentPromptConfig?.shortcuts || [])?.find(({ key }) => {
            ...
            if (data.state.shortcut === 'enter') {
              if (data?.state?.multiple) {
                ...
                submit(data?.state?.focused?.value || data?.state?.input)
              }
            }
      ...
      complete: () => {
        // global.log(
        //   `${process.pid}: ✂  Remove all handlers`
        // )
      },
    })
...
let onEscapeDefault: ChannelHandler = async (input: string, state: AppState) => {
  exit()
}

let onAbandonDefault = () => {
  global.log(`${process.pid}: Abandon caused exit. Provide a "onAbandon" handler to override.`)
  exit()
}
...
let onMenuToggleDefault = async (input, state) => {
  if (state.flaggedValue) {
    let localChoice: Choice = (global.kitPrevChoices || []).find((c: Choice) => c.id === state?.focused?.id)
    if (localChoice?.actions) {
...
let onPasteDefault = async (input, state) => {
  if (state.paste) {
    setSelectedText(state.paste, false)
  }
}
...
let prepPrompt = async (config: PromptConfig) => {
  let escapeDefault = Boolean(!config?.onEscape || config?.onEscape === onEscapeDefault)
  let hasEscapeShortcut = Boolean((config?.shortcuts || []).find((s) => s.key === 'escape'))
  ...
  if (typeof hideOnEscape === 'undefined') {
    hideOnEscape = Boolean(escapeDefault && !hasEscapeShortcut)
  }
  let choicesAreAFunction = typeof choices === 'function'
  let choicesHasAnInputArg = (choices as Function)?.length > 0

  let mode = choicesAreAFunction && choicesHasAnInputArg ? Mode.GENERATE : Mode.FILTER
  ...
  let promptData = {
    ...
    panel: panel && typeof panel === 'function' ? await (panel as Function)() : (panel as string),
    preview: preview && typeof preview === 'function' ? await (preview as Function)() : (preview as string),
    ...
    hideOnEscape,
    keyword,
    searchKeys: config?.searchKeys || ['slicedName', 'friendlyShortcut', 'tag', 'group', 'command'],
  }

  global.setPrompt(promptData as PromptData)
}

let createOnInputDefault = (choices, className, debounceInput) => {
  let choicesAreAFunction = typeof choices === 'function'
...
let createOnActionInputDefault = (actions, className, debounceInput) => {
...
global.kitPrompt = async (config: PromptConfig) => {
  ...
  promptId++
  global.__currentPromptSecret = config.secret
  ...
  if (config?.focusedId) {
    config.defaultChoiceId = config.focusedId || global.__kitFocusedChoiceId
  }

  let {
    input = '',
    inputRegex = arg?.keyword ? `(?<=${arg?.keyword}\\s)(.*)` : '',
    choices = null,
    initialChoices = null,
    className = '',
    validate = null,
    ...
    debounceInput = 200,
    onInput = createOnInputDefault(choices, className, debounceInput),
    onActionsInput = createOnActionInputDefault(config?.actions, className, debounceInput),
  ...
  let choiceFocus = createOnChoiceFocusDefault(debounceChoiceFocus, onChoiceFocus)
...
global.drop = async (placeholder = 'Drop something here...') => {
  let config: Partial<PromptConfig> = typeof placeholder === 'string' ? { placeholder } : placeholder
...
global.fields = async (formFields) => {
  ...
  let inputs = f
    .map((field, i) => {
      ...
      let { element, label, id, name, ...attrs } = defaultElement
      ...
      <div class="w-full pt-4 flex flex-col-reverse">
              ...
              <${element}
                  ...
                  <label for=${id || i} htmlFor=${id || i} class="text-sm px-4 block font-normal text-text-base text-opacity-75 peer-focus:text-primary peer-focus:text-opacity-90">
                          ${label}
                        </label>
          </div>
    ...
    .join('')
  ;(config as PromptConfig).html = `<div class="flex flex-col items-center min-h-full flex-1 w-full">
...
<input type="submit" name="submit-form" value="Submit" class="focus:underline underline-offset-4 outline-none p-3 text-contrast-dark dark:text-contrast-light text-opacity-75 dark:text-opacity-75 font-medium text-sm focus:text-text-base dark:focus:text-primary-light hover:text-text-base dark:hover:text-primary-light hover:underline dark:hover:underline bg-opacity-75 dark:bg-opacity-75"/>
  ...
  let formResponse = await global.form(config as PromptConfig)
...
let maybeWrapHtml = (html = '', containerClasses = '') => {
  return containerClasses?.length === 0 ? html : `<div class="${containerClasses}">${html}</div>`
}

global.div = async (htmlOrConfig = '', containerClasses = '') => {
  let config: {
    html?: string
  } = typeof htmlOrConfig === 'string' ? { html: htmlOrConfig } : htmlOrConfig

  if (config.html.trim() === '') {
    htmlOrConfig = md('! html string was empty')
  }
  return await global.kitPrompt({
    enter: 'Continue',
    shortcuts: [escapeShortcut],
...
global.groupMarkdownFileIntoChoices = async (filePath: string) => {
  ...
  let group = ''
  let order = []
  let useGroups = tokens.find((t) => t.type === 'heading' && t.depth === 3)
  ...
  for (let token of tokens) {
    ...
    if (token.type === 'heading' && token.depth === 2 && useGroups) {
      group = token.text
    ...
    } else if (token.type === 'html' && token.text.startsWith('<!--')) {
      // Fallback to h2/h1s
      ...
      if (sections.length) {
        sections[sections.length - 1].comments[key.trim()] = trimmedValue
      }
  ...
  let choices = sections.map((section) => {
    ...
    return {
      name: section.name,
      className: 'text-base',
      preview: async () => highlight(section.raw, containerClasses),
      value,
      ...section.comments,
      group: section?.group,
    }
  ...
  let groupedChoices = groupChoices(choices, {
    order,
    sortChoicesKey: Array.from({ length: order.length }, () => false),
  })
...
global.docs = async (filePath: string, options = {}) => {
  let { placeholder, groupedChoices, sections, tokens } = await global.groupMarkdownFileIntoChoices(filePath, options)

  let config = typeof options === 'function' ? await options(sections, tokens) : options
...
global.editor = async (options?: EditorOptions) => {
  ...
  let editorOptions =
    typeof options === 'string' ? { ...defaultOptions, value: options } : { ...defaultOptions, ...options }
...
global.editor.setSuggestions = async (suggestions: string[] = []) => {
  await sendWait(Channel.SET_EDITOR_SUGGESTIONS, suggestions)
}
...
global.editor.getCursorOffset = async () => {
  let message = await sendWait(Channel.EDITOR_GET_CURSOR_OFFSET)
  return message?.state?.value
}

global.editor.moveCursor = async (offset: number) => {
  let message = await sendWait(Channel.EDITOR_MOVE_CURSOR, offset)
  return message?.state?.value
}

global.editor.insertText = async (text: string) => {
  let message = await sendWait(Channel.EDITOR_INSERT_TEXT, text)
  return message?.state?.value
}
...
global.template = async (template = '', options: EditorOptions = { language: 'plaintext' }) => {
...
global.hotkey = async (placeholder = 'Press a key combo:') => {
  let config = typeof placeholder === 'string' ? { placeholder } : placeholder
...
global.basePrompt = async (placeholderOrConfig = 'Enter text...', choices = '', actions = '') => {
  if (typeof placeholderOrConfig === 'object' && placeholderOrConfig?.choices) {
  ...
  let hint = (placeholderOrConfig as PromptConfig)?.hint || ''
  ...
  if (typeof firstArg !== 'undefined' && firstArg !== null) {
    ...
    if (typeof validate === 'function') {
      ...
      hint = typeof valid === 'boolean' && !valid ? `${firstArg} is not valid` : (valid as string)
    } else {
      return firstArg
    }
  }
  ...
  if (!choices) {
    height = PROMPT.HEIGHT.HEADER + PROMPT.INPUT.HEIGHT.SM + PROMPT.HEIGHT.FOOTER
  }
  if (typeof placeholderOrConfig === 'object') {
    let { headerClassName = '', footerClassName = '', inputHeight } = placeholderOrConfig as PromptConfig
    if (inputHeight) {
      height = inputHeight
    }
  ...
  let promptConfig: PromptConfig = {
    ...
    hint,
    height,
    resize: choices ? undefined : true,
    shortcuts: (placeholderOrConfig as PromptConfig)?.resize ? smallShortcuts : argShortcuts,
    choices,
    actions,
    preview: typeof actions === 'string' ? actions : undefined,
  }

  if (Array.isArray(choices) && !(choices as Choice[]).find((c) => c?.preview)) {
...
global.select = async (placeholderOrConfig = 'Type a value:', choices = [], actions = []) => {
  let config: PromptConfig = {
    multiple: true,
    enter: 'Select',
    shortcuts: [
...
global.mini = async (placeholderOrConfig = 'Type a value:', choices = '', actions = '') => {
  let miniConfig = {
  ...
  return await global.basePrompt(miniConfig, choices, actions)
}

global.micro = async (placeholderOrConfig = 'Type a value:', choices = '', actions = '') => {
  let microConfig = {
    headerClassName: 'hidden',
    footerClassName: 'hidden',
  ...
  return await global.basePrompt(microConfig, choices, actions)
}

global.arg = process?.env?.KIT_MAIN_SCRIPT === 'v1' ? global.basePrompt : global.mini
...
global.chat.pushToken = async (token = '') => {
  await sendWait(Channel.CHAT_PUSH_TOKEN, token)
}

global.chat.setMessage = async (index: number, message = '') => {
  if (typeof message === 'string') {
    message = { text: message }
  }
  let messageDefaults = {
...
global.textarea = async (options = '') => {
  let config = typeof options === 'string' ? { value: options } : options
...
global.updateArgs = (arrayOfArgs) => {
  ...
  global.flag._ = undefined
}

global.updateArgs(process.argv.slice(2))

export let appInstallMultiple = async (packageNames: string[]) => {
  let adjustedPackageNames = packageNames.map(adjustPackageName)
  ...
  for await (let packageName of adjustedPackageNames) {
    ...
    let stripVersion = packageName.replace(/(?<=.)(@|\^|~).*/g, '')
    let packageLink = `https://npmjs.com/package/${stripVersion}`

    let response = null
    try {
      response = await get<{ downloads: number }>('https://api.npmjs.org/downloads/point/last-week/' + stripVersion)
    } catch (error) {}

    let downloads = response?.data?.downloads || 'an unknown number of'

    let hint = md(`A script requires the following packages. Do you trust them?
* ${adjustedPackageNames
      .map((name, index) => (index === i ? `<span class="text-primary font-bold">${name}</span>` : name))
      .join('\n* ')}
...
`)
    ...
    let preview =
      hint + md(`[${stripVersion}](${packageLink}) has had ${downloads} downloads from npm in the past week`)
  ...
  setHint(`Installing ${adjustedPackageNames.join(', ')}...`)
...
export let appInstall = async (packageName) => {
  ...
  if (!global.arg?.trust) {
    ...
    let preview = md(`[${stripVersion}](${packageLink}) has had ${downloads} downloads from npm in the past week`)
...
let { createNpm, createKenvPackageMissingInstall } = await import('../api/npm.js')
global.npm = createNpm(appInstall)
global.npmInstall = createNpm(appInstall, false)
global.installMissingPackage = createKenvPackageMissingInstall(appInstall, false)
...
global.getDataFromApp = global.sendWait = async (channel: GetAppData, data?: any, timeout = 1000) => {
  if (process?.send) {
    return await new Promise((res, rej) => {
      ...
      let messageHandler = (data) => {
        ...
        // }
        if (data.channel === channel) {
          res(typeof data?.value === 'undefined' ? data : data?.value)
          if (timeoutId) {
      ...
      if (timeout) {
        timeoutId = setTimeout(() => {
          ...
          rej(
            new Error(
              `Timeout after ${timeout / 1000} seconds waiting for ${channel} response
...
global.sendWaitLong = async (channel: GetAppData, data?: any, timeout = 60000) => {
  return await global.sendWait(channel, data, timeout)
}

global.getBackgroundTasks = () => global.getDataFromApp(Channel.GET_BACKGROUND)

global.getSchedule = () => global.getDataFromApp(Channel.GET_SCHEDULE)
...
global.getCurrentScreen = async () => {
  let data = await global.getDataFromApp(Channel.GET_SCREEN_INFO)
  return data?.screen
}

global.getScriptsState = () => global.getDataFromApp(Channel.GET_SCRIPTS_STATE)
...
global.getClipboardHistory = async () => {
  ...
  let clipboardHistory = (await clipboardStore.get('history')) as ClipboardItem[]
...
global.setDescription = async (description: string) => {
  await global.sendWait(Channel.SET_DESCRIPTION, description)
}
...
for (const method of loadingList) {
  ...
  if (global.hasOwnProperty(method)) {
    ...
    global[method] = function (...args: any[]) {
      ...
      return Promise.resolve(result).finally(() => setLoading(false))
    }
  }
}
...
global.mainScript = async (input: string, tab: string) => {
  if (arg?.keyword) {
...
export let createPathChoices = async (
  ...
  for (let dirent of dirents) {
    if (dirent.isSymbolicLink()) {
      let resolved
      try {
        resolved = await fs.promises.realpath(path.resolve(dirent.path, dirent.name))
      } catch (error) {
  ...
  let folders = dirents.filter((dirent) => dirent.isDirectory())
  let files = onlyDirs ? [] : dirents.filter((dirent) => !dirent.isDirectory())
  ...
  let mapDirents = (dirents: Dirent[]): Choice[] => {
    return dirents.map((dirent) => {
      ...
      let description = type === 'folder' ? '' : `${filesize(size)} - Last modified ${formatDistanceToNow(mtime)} ago`

      let img = pathToFileURL(kitPath('icons', type + '.svg')).href
...
let __pathSelector = async (config: string | PathConfig = home(), { showHidden } = { showHidden: false }) => {
  let startPath = ''
  let focusOn = ''
...
global.getEditorHistory = async () => {
  return (await global.getDataFromApp(Channel.GET_EDITOR_HISTORY))?.state?.editorHistory
}
...
global.clipboard = {
  ...
  readImage: async () => {
    let tmpPath = await sendWait(Channel.CLIPBOARD_READ_IMAGE)
  ...
  writeText: async (text: string) => {
    return await sendWait(Channel.CLIPBOARD_WRITE_TEXT, text)
  },
  writeHTML: async (html: string) => {
    return await sendWait(Channel.CLIPBOARD_WRITE_HTML, html)
  },
  writeImage: async (image: Buffer) => {
    let imagePath = tmpPath(`${uuid()}.png`)
    await writeFile(imagePath, image)
    return await sendWait(Channel.CLIPBOARD_WRITE_IMAGE, imagePath)
  },
  ...
  writeBookmark: async (bookmark: {
    ...
    return await sendWait(Channel.CLIPBOARD_WRITE_BOOKMARK, bookmark)
  },
  writeFindText: async (findText: string) => {
    return await sendWait(Channel.CLIPBOARD_WRITE_FIND_TEXT, findText)
  },
...
let addNodeLibs = async () => {
  ...
  let nodeTypesDir = kitPath('node_modules', '@types', 'node')
  ...
  for await (let dirent of nodeDirents) {
    if (dirent.isDirectory()) {
      let { name } = dirent
      let subDirent = await readdir(path.resolve(nodeTypesDir, name), {
        withFileTypes: true,
      })

      for await (let sub of subDirent) {
        if (sub.isFile() && sub.name.endsWith('.d.ts')) {
          let filePath = path.resolve(nodeTypesDir, name, sub.name)
      ...
      if (name.endsWith('d.ts')) {
        let content = await readFile(kitPath('node_modules', '@types', 'node', name), 'utf8')
        extraLibs.push({
          content,
          filePath: `file:///${name}`,
        })
...
let addKitLibs = async (): Promise<ExtraLib[]> => {
  ...
  for await (let t of kitCoreTypes.filter((t) => t.endsWith('.d.ts'))) {
  ...
  for await (let t of kitTypes) {
    let content = await readFile(kitPath('types', t), 'utf8')
  ...
  let globalTypesDir = kitPath('node_modules', '@johnlindquist', 'globals', 'types')

  let globalTypeDirs = (await readdir(globalTypesDir, { withFileTypes: true })).filter((dir) => dir.isDirectory())

  for await (let { name } of globalTypeDirs) {
    let content = await readFile(
      kitPath('node_modules', '@johnlindquist', 'globals', 'types', name, 'index.d.ts'),
      'utf8',
    )
  ...
  let globalsIndexContent = await readFile(
    kitPath('node_modules', '@johnlindquist', 'globals', 'types', 'index.d.ts'),
    'utf8',
  )
  ...
  let content = await readFile(kitPath('editor', 'types', 'kit-editor.d.ts'), 'utf8')
  extraLibs.push({
    content,
    filePath: 'file:///kit.d.ts',
  })

  let shelljsContent = await readFile(kitPath('node_modules', '@types', 'shelljs', 'index.d.ts'), 'utf8')
  ...
  let nodeNotifierContent = await readFile(kitPath('node_modules', '@types', 'node-notifier', 'index.d.ts'), 'utf8')
...
global.__kit__onShortcutPressed = async (input: string, state: AppState) => {
...
global.registerShortcut = async (shortcut: string, callback: () => void) => {
  if (process?.send) {
    let result = await sendWait(Channel.REGISTER_GLOBAL_SHORTCUT, shortcut)

    log({ result })
    if (!result) {
      warn(`Shortcut ${shortcut} failed to register. Ending process. 😰`)
      exit()
    }
    let messageHandler = (data: any) => {
      if (data.channel === Channel.GLOBAL_SHORTCUT_PRESSED && data.value === shortcut) {
        callback()
      }
    }
...
global.unregisterShortcut = async (shortcut: string) => {
  ...
  let messageHandler = __kit__registeredShortcuts.get(shortcut)
  if (messageHandler) {
    process.off('message', messageHandler)
...
global.startDrag = async (filePath: string, iconPath: string) => {
...
let beginMicStream = () => {
  ...
  if (!process.listeners('message').includes(micStreamHandler)) {
...
let micStreamHandler = (message: AppMessage) => {
  if (message.channel === Channel.MIC_STREAM) {
    ...
    if (message?.state?.value) {
      ...
      const buffer = isBuffer ? message.state?.value : Buffer.from(message.state?.value)

      if (global.mic.stream) {
        global.mic.stream.push(buffer)
      }
    }
  }
...
global.mic.start = async (config: MicConfig) => {
  beginMicStream()
  let filePath = tmpPath(global.formatDate(new Date(), 'yyyy-MM-dd_HH-mm-ss') + '.webm')
...
global.setSelectedChoices = async (choices: Choice[]) => {
  return await sendWait(Channel.SET_SELECTED_CHOICES, choices)
}
